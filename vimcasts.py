...
class Episode(object):
    ...
    def save(self, formatstr='{number}. {title}.{ext}'):
        ...
        """
        path = formatstr.format(number = self.number,
                                title  = self.title,
                                ext    = self.ext)
...
class Episodes(object):
    ...
    def __iter__(self):
        """Parse a xml feed, put them in property and iterate over results."""
...
#            print("Opened XML: {}".format(xml))
        except urllib2.URLError:
            xml = ''
        return self.parse_feed(xml)

    def parse_feed(self, xml):
        """Used by feed property"""

        _feed_re = re.compile('\<item\>(.*?)\</item\>', re.DOTALL)
        _item_re = re.compile('\<title\>(.*?)\</title\>(?=.*?\<enclosure url=\"(.*?)\").*?\<pubDate\>(.*?)\</pubDate\>', re.DOTALL)

        res = [ x for item in _feed_re.findall(xml)
                    for x in _item_re.findall(item) ]

        res = sorted(res,
                     key=lambda x: datetime.strptime(x[2], '%a, %d %b %Y %H:%M:%S %Z'))
    ...
    def get_feed_url(self, video_format):
        """e.g. http://vimcasts.org/feeds/quicktime.rss """
        url = self.feedformatstr.format(video_format=video_format)
